...
{
    OE_NOTICE << std::endl << "Usage: " << name << " --odr geoReferencedOpenDrive.xodr" << " file.earth" << std::endl
...
main(int argc, char** argv)
{
    ...
    // OpenDrive starts
    osg::ref_ptr<osg::Group> model = LoadOdr(arguments, mapNode);
    if(model.get() == nullptr)
    {
        OE_INFO << "Could not load OpenDrive. Exit" << std::endl;
        return -1;
    }
    Viewpoint vp = OdrRefPoint();

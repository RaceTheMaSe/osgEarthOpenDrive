...
REGISTER_OSGEARTH_LAYER(flattenedelevation, FlatteningLayerAlternative);
REGISTER_OSGEARTH_LAYER(flattened_elevation, FlatteningLayerAlternative);

#define LC "[FlatteningLayerAlternative] "
...
Config
FlatteningLayerAlternative::Options::getConfig() const
{
...
void
FlatteningLayerAlternative::Options::fromConfig(const Config& conf)
{
    fill().init(false);
    lineWidth().init(40);
    bufferWidth().init(40);
...
OE_LAYER_PROPERTY_IMPL(FlatteningLayerAlternative, NumericExpression, LineWidth, lineWidth);
OE_LAYER_PROPERTY_IMPL(FlatteningLayerAlternative, NumericExpression, BufferWidth, bufferWidth);
OE_LAYER_PROPERTY_IMPL(FlatteningLayerAlternative, bool, Fill, fill);

void
FlatteningLayerAlternative::init()
{
    ElevationLayer::init();
...
Config
FlatteningLayerAlternative::getConfig() const
{
...
Status
FlatteningLayerAlternative::openImplementation()
{
...
FlatteningLayerAlternative::~FlatteningLayerAlternative()
{
    //nop
}

void
FlatteningLayerAlternative::setFeatureSource(FeatureSource* layer)
{
    options().featureSource().setLayer(layer);
}

void
FlatteningLayerAlternative::addedToMap(const Map* map)
{
...
void
FlatteningLayerAlternative::removedFromMap(const Map* map)
{
...
FeatureList
FlatteningLayerAlternative::getFeatures(const TileKey& key)
{
...
inline void addFeatureIfRelevant(const FlatteningLayerAlternative* fl, Feature* feature, osg::ref_ptr< Session >& session, const SpatialReference* workingSRS, const GeoExtent& geoExtent, const GeoExtent& queryExtent, bool needsTransform, MultiGeometry& geoms, WidthsList& widths)
{
...
GeoHeightField
FlatteningLayerAlternative::createHeightFieldImplementation(const TileKey& key, ProgressCallback* progress) const
{
    ...
    if (featureProfile->getTilingProfile())
    {
        ...
        {
            lists.emplace_back(std::move(const_cast<FlatteningLayerAlternative*>(this)->getFeatures(featureKey)));

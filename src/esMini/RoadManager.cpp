...
bool OpenDrive::LoadOpenDriveFile(const char* filename, bool replace, bool verbose_logging)
{
    if (replace)
    {
        Clear();
                ...
                if (!strcmp(signal.name(), "signal"))
                {
                    ...
                    if (!type.empty())
                    {
                        ...
                        if (osi_type == static_cast<int>(Signal::OSIType::TYPE_UNKNOWN))
                        {
                            ...
                            if (osi_type == static_cast<int>(Signal::OSIType::TYPE_UNKNOWN) && verbose_logging)
                            {
...
void OpenDrive::ParseGeoLocalization(const std::string& geoLocalization)
{
    ...
    geo_ref_.rawString = geoLocalization;
}
        ...
        for (int j = 0; j < number_of_lane_sections; j++)
        {
            ...
            for (int k = 0; k < number_of_lanes; k++)
            {
                ...
                while (++counter)
                {
                    ...
                    // If requirement is satisfied -> look for further points

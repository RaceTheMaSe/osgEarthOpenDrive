...
double zOffset = 0.01;  // different to esMini original code! just offset the geometry to mitigate z-fighting to not rely on a scene graph feature to do it

void ConvertGeomPointToMapCoordinates(RoadGeom::GeomPoint& p)
{
    osg::Vec3d geoCentric = ConvertToMapCoordinatesXYZ(p.x,p.y,p.z);
    p.x = geoCentric.x();
    p.y = geoCentric.y();
    p.z = geoCentric.z();
}
 
void RoadGeom::convertGeometryPointListToMapCoords(std::vector<std::vector<GeomPoint>>& geom_point_list)
{
    for (auto& slice : geom_point_list)
    {
        for(auto& p : slice)
            ConvertGeomPointToMapCoordinates(p);
    }
}

void RoadGeom::initMaterials()
{
    size_t matId = size_t(roadmanager::RoadMarkColor::UNKNOWN);
    materialsRoadmark_.resize(matId);
    materials_colors_.resize(matId);
    for (size_t i = 0; i < matId; ++i)
    {
        osg::ref_ptr<osg::Material>&  mat    = materialsRoadmark_[i];
        osg::ref_ptr<osg::Vec4Array>& colors = materials_colors_[i];
        mat                                  = new osg::Material;
        colors                               = new osg::Vec4Array;
        colors->push_back(ODR2OSGColor(roadmanager::RoadMarkColor(i)));
        mat->setDiffuse(osg::Material::FRONT_AND_BACK, colors->at(0));
        mat->setAmbient(osg::Material::FRONT_AND_BACK, colors->at(0));
        mat->setShininess(osg::Material::FRONT_AND_BACK, 0);
    }
    tex_asphalt = ReadTexture("uv-coords.jpg");
    tex_grass   = ReadTexture("grass.jpg");

    color_asphalt      = new osg::Vec4Array;
    color_concrete     = new osg::Vec4Array;
    color_border_inner = new osg::Vec4Array;
    color_grass        = new osg::Vec4Array;

    materialAsphalt_     = new osg::Material;
    materialGrass_       = new osg::Material;
    materialConcrete_    = new osg::Material;
    materialBorderInner_ = new osg::Material;
...
// the following two functions are from some project doing similar things less efficiency by having duplicate vertices at each repeating point ... the main benefit is road aligned UVs
// but that is available and essentially how open drive roads are defined along a road aligned coordiante system with s and t ... so maybe completely unnecessary .. or can be implemented better
void VertexDefaultNormalsTangentColors(osg::ref_ptr<osg::Vec3Array> &Normals, osg::ref_ptr<osg::Vec4Array> &VertexColors, osg::ref_ptr<osg::Vec3Array> &Tangents, size_t Amount)
{
	for (size_t Index = 0; Index < Amount; Index++)
	{
		Normals->push_back(osg::Vec3d(0, 0, 1));
		VertexColors->push_back(osg::Vec4d(255, 165, 0, 100));
		Tangents->push_back(osg::Vec3d(1, 1, 1));
	}
}

void meshAndUvRoadSection(std::vector<std::vector<RoadGeom::GeomPoint>>& AllRoadSlices)
{
	// boundary to mesh algo
	osg::ref_ptr<osg::Vec3Array> Vertices;
	osg::ref_ptr<osg::Vec3Array> Normals;
	osg::ref_ptr<osg::Vec2Array> TextureCoordinates;
	osg::ref_ptr<osg::IntArray> Triangles;
	osg::ref_ptr<osg::Vec4Array> VertexColors;
    osg::ref_ptr<osg::Vec3Array> Tangents;

	int32_t TriangleOffset = 0;
	const float TextureLength = 1.0f; // 1m tileable asphalt texture
	const float TextureWidth = 1.0f; // 1m tileable asphalt texture
	float CurrentSampleDistance = 0.0f;

	if (AllRoadSlices.size() > 1)
	{
		for (int32_t SliceIndex = 0; SliceIndex < int32_t(AllRoadSlices.size() - 1); SliceIndex++)
		{
			auto CurrentSlice = AllRoadSlices[SliceIndex];
			auto NextSlice = AllRoadSlices[SliceIndex + 1];

			for (int32_t Point = 0; Point < (CurrentSlice.size() - 1); Point++)
			{
				if (CurrentSlice.size() == NextSlice.size())
				{
					auto A = CurrentSlice[Point];
					auto B = CurrentSlice[Point + 1];
					auto C = NextSlice[Point + 1];
					auto D = NextSlice[Point];

                    auto Av = osg::Vec3d(A.x,A.y,A.z);
                    auto Bv = osg::Vec3d(B.x,B.y,B.z);
                    auto Cv = osg::Vec3d(C.x,C.y,C.z);
                    auto Dv = osg::Vec3d(D.x,D.y,D.z);

					auto Dist = (Dv-Av).length();
					auto WidthStart = (Bv-Av).length();
					auto WidthEnd = (Dv-Cv).length();
					auto TextureU = WidthStart / TextureWidth;
					auto TextureU2 = WidthEnd / TextureWidth;
					auto TextureV = CurrentSampleDistance / TextureLength;
					auto TextureV2 = CurrentSampleDistance + Dist / TextureLength;
					CurrentSampleDistance += Dist;

					// add quad of defining road (or lane) boundary
					Vertices->push_back(Av);
					Vertices->push_back(Bv);
					Vertices->push_back(Cv);
					Vertices->push_back(Dv);

					// *4 because of two triangles
					Triangles->push_back(TriangleOffset + 0);
					Triangles->push_back(TriangleOffset + 1);
					Triangles->push_back(TriangleOffset + 2);
					Triangles->push_back(TriangleOffset + 2);
					Triangles->push_back(TriangleOffset + 3);
					Triangles->push_back(TriangleOffset + 0);

					TextureCoordinates->push_back(osg::Vec2d(0, TextureV));
					TextureCoordinates->push_back(osg::Vec2d(TextureU, TextureV));
					TextureCoordinates->push_back(osg::Vec2d(TextureU2, TextureV2));
					TextureCoordinates->push_back(osg::Vec2d(0, TextureV2));

					VertexDefaultNormalsTangentColors(Normals, VertexColors, Tangents, 4);
...
void RoadGeom::roadAndMarkingsMesh(std::vector<std::vector<GeomPoint>>& geom_point_list,
                                   ...
                                   roadmanager::Position&               pos)
{
    ...
    for (size_t k = 0; k < static_cast<unsigned int>(lsec->GetNumberOfLanes()); k++)
    {
        ...
        {
            const GeomPoint& gp = geom_point_list[l][k];
        ...
        if (k > 0)
        {
            ...
            AddNodeWithLod(road_group_, geode);
            geode->computeBound();
        }
        AddRoadMarks(lane, base_);
        // NOTE: performance is rather bad in the current implementation ... probably because its so many objects that need to be culled and constant state changes
        // there are several ideas to how to make this more performant:
        // - remove state changes from vertex colors and use a texture that holds one pixel for each of the colors thats desired, assigning appropiate UV coords, so no texture change needs to be solid color objects,
        // - reduce amount of objects to only one per lane section - should be enough to cull them in groups and maybe even by distance from viewer ... as lane markers are likely irrelevant when kilometers away
    }
}

void RoadGeom::laneSectionOutline(std::vector<std::vector<GeomPoint>>& geom_point_list, unsigned int numLanes, int road_id, int lane_id)
{
    // road outline - iterate forward on first lane border -> end cap -> back on outermost lane border -> start cap
    unsigned int                  nrOfVerticesOutline = static_cast<unsigned int>(geom_point_list.size()) * 2 + (numLanes - 2) * 2 + 1;
    osg::ref_ptr<osg::Vec3dArray> verticesOutline     = new osg::Vec3dArray(nrOfVerticesOutline);
    unsigned int                  vidxOutlineFirst    = 0;
    unsigned int                  vidxOutlineSecond   = (nrOfVerticesOutline - 1) - (numLanes - 2);
    const size_t                  numSlices           = geom_point_list.size();
    const size_t                  firstSlice          = 0;
    const size_t                  lastSlice           = numSlices - 1;
    for (size_t k = 0; k < numLanes; k++)
    {
        for (size_t l = 0; l < numSlices; l++)
        {
            const GeomPoint& gp = geom_point_list[l][k];
            if (k == 0)
                (*verticesOutline)[vidxOutlineFirst++].set(gp.x, gp.y, gp.z);
            else if (k == numLanes - 1)
                (*verticesOutline)[vidxOutlineSecond--].set(gp.x, gp.y, gp.z);
            if (l == firstSlice)
            {
                if (k > 0 && k < numLanes)
                    (*verticesOutline)[static_cast<unsigned int>(nrOfVerticesOutline - 1 - k)].set(gp.x, gp.y, gp.z);
            }
            else if (l == lastSlice)
            {
                if (k > 0 && k < numLanes)
                    (*verticesOutline)[static_cast<unsigned int>(lastSlice + k)].set(gp.x, gp.y, gp.z);
            }
        }
    }
    // close outline
    const GeomPoint& gp = geom_point_list[0][0];
    (*verticesOutline)[nrOfVerticesOutline - 1].set(gp.x, gp.y, gp.z);
    // HACK: invert the order as after implementing it got obvious that the cutout geometry in osgEarth layers requires CCW and not CW ...
    // remove and re-implement the above
    osg::ref_ptr<osg::Vec3dArray> verticesOutlineReversed = new osg::Vec3dArray(nrOfVerticesOutline);
    size_t                        i2                      = 0;
    for (int irev = static_cast<int>(nrOfVerticesOutline) - 1; irev > -1; --irev, ++i2)
        (*verticesOutlineReversed)[static_cast<osg::MixinVector<osg::Vec3d>::size_type>(irev)] = (*verticesOutline)[i2];
    outlines[(road_id << 8) | (lane_id & 0xFF)] = verticesOutlineReversed;
}

void RoadGeom::createLaneSectionSlice(std::vector<std::vector<GeomPoint>>& geom_point_list,
                                      ...
                                      int                                  road_id)
{
    std::vector<GeomPoint> geom_point;  // current lane geometry
    ...
    for (int counter = 1; !done && counter > 0; counter++)
    {
        ...
        else
        {
            ...
            for (size_t k = 0; k < static_cast<unsigned int>(lsec->GetNumberOfLanes()); k++)
            {
                ...
                for (; l < osiPoints.size(); l++)
                {
                    ...
                    pos.SetTrackPos(road_id, osiPoints[l].s, SIGN(lane->GetId()) * lsec->GetOuterOffset(osiPoints[l].s, lane->GetId()), true);
        ...
        for (int m = 0; m < n; m++)
        {
            ...
            {
                lane = lsec->GetLaneByIdx(static_cast<int>(k));
                pos.SetTrackPos(road_id, s, SIGN(lane->GetId()) * lsec->GetOuterOffset(s, lane->GetId()), true);
...
void RoadGeom::AddRoadMarkGeom(osg::ref_ptr<osg::Vec3Array> vertices, osg::ref_ptr<osg::DrawElementsUInt> indices, roadmanager::RoadMarkColor color)
{
    ...
    geom->addPrimitiveSet(indices.get());
    geom->setColorArray(materials_colors_[size_t(color)]);
    geom->setColorBinding(osg::Geometry::BIND_OVERALL);

    // Use PolygonOffset feature to avoid z-fighting with road surface
    // geom->getOrCreateStateSet()->setAttributeAndModes(new osg::PolygonOffset(-2.0, -1.0)); // deactivated for fixed zOffset
    osgUtil::SmoothingVisitor::smooth(*geom, 0.0);

    osg::ref_ptr<osg::Geode> geode = new osg::Geode;
    geode->addDrawable(geom);
    geode->getOrCreateStateSet()->setAttributeAndModes(materialsRoadmark_[size_t(color)]);
    AddNodeWithLod(rm_group_,geode);
}
            ...
            for (int n = 0; n < lane_roadmarktype->GetNumberOfRoadMarkTypeLines(); n++)
            {
                ...
                if (lane_roadmark->GetType() == roadmanager::LaneRoadMark::RoadMarkType::BOTTS_DOTS)
                {
                    for (unsigned int q = 0; q < curr_osi_rm->GetPoints().size(); q++)
                    {
                        ...
                        tx->setPosition(ConvertToMapCoordinatesXYZ(osi_point0.x,osi_point0.y,osi_point0.z));
                        tx->addChild(dot);
                        AddNodeWithLod(rm_group_,tx);
                    }
                }
                         ...
                         lane_roadmark->GetType() == roadmanager::LaneRoadMark::RoadMarkType::BROKEN_BROKEN || broken)
                {
                    for (unsigned int q = 0; q < curr_osi_rm->GetPoints().size(); q += 2)
                    {
                        ...
                        (*vertices)[0].set(ConvertToMapCoordinatesXYZ(x0l,y0l,osi_point0.z + zOffset));
                        (*vertices)[1].set(ConvertToMapCoordinatesXYZ(x0r,y0r,osi_point0.z + zOffset));
                        (*vertices)[2].set(ConvertToMapCoordinatesXYZ(x1l,y1l,osi_point1.z + zOffset));
                        (*vertices)[3].set(ConvertToMapCoordinatesXYZ(x1r,y1r,osi_point1.z + zOffset));
                         ...
                         lane_roadmark->GetType() == roadmanager::LaneRoadMark::RoadMarkType::SOLID_SOLID || !broken)
                {
                    ...
                    for (size_t q = 0; q < osi_points.size(); q++)
                    {
                        ...
                        if (q == 0)
                        {
                            ...
                            (*vertices)[q * 2 + 0].set(ConvertToMapCoordinatesXYZ(l1p0l[0],l1p0l[1],osi_points[q].z + zOffset));
                            (*vertices)[q * 2 + 1].set(ConvertToMapCoordinatesXYZ(l1p0r[0],l1p0r[1],osi_points[q].z + zOffset));
                        }
                        else if (q == osi_points.size() - 1)
                        {
                            // Last point, no adjustment needed
                            (*vertices)[q * 2 + 0].set(ConvertToMapCoordinatesXYZ(l1p1l[0],l1p1l[1],osi_points[q].z + zOffset));
                            (*vertices)[q * 2 + 1].set(ConvertToMapCoordinatesXYZ(l1p1r[0],l1p1r[1],osi_points[q].z + zOffset));
                        }
                        else
                        {
                            ...
                            if (GetIntersectionOfTwoLineSegments(l0p0l[0],
                                                                 ...
                                                                 isect[1]) == 0)
                            {
                                (*vertices)[q * 2 + 0].set(ConvertToMapCoordinatesXYZ(isect[0],isect[1],osi_points[q].z + zOffset));
                            }
                            else
                            {
                            ...
                            if (GetIntersectionOfTwoLineSegments(l0p0r[0],
                                                                 ...
                                                                 isect[1]) == 0)
                            {
                                (*vertices)[q * 2 + 1].set(ConvertToMapCoordinatesXYZ(isect[0],isect[1],osi_points[q].z + zOffset));
                            }
                            else
                            {
                                // lines parallel, no adjustment needed
...
RoadGeom::RoadGeom(roadmanager::OpenDrive* odr, osg::Matrixd originGeoCentric)
{
    // root (group)
    //   base (matrix) // essential to mitigate floating point precision issues when rendering
    //     road_group_ (group)
    //       [road geodes]
    //     rm_group_ (group)
    //       [road marking geodes]
    root_       = new osg::Group;
    road_group_ = new osg::Group;
    rm_group_   = new osg::Group;
    base_       = new osg::MatrixTransform(originGeoCentric);
    root_->addChild(base_);
    base_->addChild(road_group_);
    base_->addChild(rm_group_);
    ...
    for (size_t i = 0; i < static_cast<unsigned int>(odr->GetNumOfRoads()); i++)
    {
        ...
        for (size_t j = 0; j < static_cast<unsigned int>(road->GetNumberOfLaneSections()); j++)
        {
            ...
            if (lsec->GetNumberOfLanes() < 2)
            {
                ...
                LOG("Skipping empty lane section (reference line only) %u on road %u", j, i);
                continue;
            }
            ...
            // a.k.a vector of road slice / cross sections / lane sections / curvature and
            // elevation profile-dependent sampled s-values
            std::vector<std::vector<GeomPoint>> geom_point_list;
            s_list.clear();
            // be aware that the positions calculated here are in the cartesian odr reference frame and need to be converted with the geoReference coordinates to obtain correct earth positions ... if there is a geoReference and this is a map derived from this earth
            roadmanager::Position pos;
            int                   road_id = road->GetId();
            createLaneSectionSlice(geom_point_list, s_list, pos, lsec, lane, road_id);
            laneSectionOutline(geom_point_list, lsec->GetNumberOfLanes(), road_id, (int)j);
            // additional step not in esMini: convert those points from OpenDrive local cartesian coordinate frame to map coordinates - most likely wgs84
            convertGeometryPointListToMapCoords(geom_point_list);
            // Then create actual vertices and triangle strips for the lane section
            roadAndMarkingsMesh(geom_point_list, lsec, lane, pos);
        }
    }
}

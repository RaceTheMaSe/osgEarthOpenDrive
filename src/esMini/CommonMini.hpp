...
public:
    ...
    bool                            GetOptionSet(std::string opt) const;
    bool                            IsOptionArgumentSet(std::string opt) const;
    std::string                     GetOptionArg(std::string opt, int index = 0) const;
    int                             ParseArgs(int argc, const char* const argv[]);
    const std::vector<std::string>& GetOriginalArgs() const
    {
        return originalArgs_;
    }
    bool IsInOriginalArgs(std::string opt) const;
...
private:
    ...
    const SE_Option* GetOption(std::string opt) const;

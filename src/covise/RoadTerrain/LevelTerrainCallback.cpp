...
LevelTerrainCallback::LevelTerrainCallback(osg::Vec3d                                    offset_,
                                           const std::vector<BoundingArea>&              areaVector,
                                           const std::vector<osg::ref_ptr<osg::Geode> >& shapeVec)
    : offset(offset_),
...
void LevelTerrainCallback::loaded(osgTerrain::TerrainTile* tile, const osgDB::ReaderWriter::Options* options) const
{
    ...
    for (int i = 0; i < vehicleUtil::RoadSystem::Instance()->getNumRoads(); ++i)
    {
        ...
        if (((roadBB.xMax() >= tileXMin && roadBB.xMax() <= tileXMax) || (roadBB.xMin() >= tileXMin && roadBB.xMin() <= tileXMax) ||
             (roadBB.xMin() <= tileXMin && roadBB.xMax() >= tileXMax)) &&
            (roadBB.yMax() >= tileYMin && roadBB.yMin() <= tileYMax))
        {
            ...
            for (double s_ = h; s_ < roadLength + h; s_ = s_ + h)
            {
                ...
                for (std::map<int, std::map<int, double> >::iterator yScanlineIt = fillBorderMap.begin(); yScanlineIt != fillBorderMap.end();
                     ++yScanlineIt)
                {
                    ...
                    for (int x = x0; x <= x1; ++x)
                    {
                        if (x >= 0 && x < (int)field->getNumColumns() && y >= 0 && y < field->getNumRows())
                        {
                            ...
                            double l  = sqrt(pow(0.5 * (pointDeque[0].x() - pointDeque[2].x() + pointDeque[1].x() - pointDeque[3].x()), 2) +
                                            pow(0.5 * (pointDeque[0].y() - pointDeque[2].y() + pointDeque[1].y() - pointDeque[3].y()), 2));
                            ...
                            // std::cout << "(x,y): (" << x << "," << y << "), zt: " << zt << ", zf: " << zf << ", kt: " << kt << ", kf: " << kf <<
                            // std::endl;

                            // field->setHeight(x,y, std::min(zf, 0.5*(z0+z1) - 1.0*(1.0- d/(wl+(wu-wl)*s/l)) ) );
                            // std::cout << "(x,y): (" << x << "," << y << "), z0: " << z0 << ", z1: " << z1 << ", d: " << d << ", wl: " << wl << ",
                            // wu: " << wu << ", s: " << s << ", l: " << l << std::endl;
                        }
                    }
                }

                vehicleUtil::Vector3D roadCenterPoint = road->getCenterLinePoint(s);
    ...
    for (int layerIt = 0; layerIt < layers.size(); ++layerIt)
    {
        ...
        while ((poFeature = layer->GetNextFeature()) != NULL)
        {
            ...
            if (poGeometry != NULL && wkbFlatten(poGeometry->getGeometryType()) == wkbPolygon)
            {
                ...
                {
                    std::cout << "Point " << pointIt << ": (" << poRing->getX(pointIt) + offset.x() << ", " << poRing->getY(pointIt) + offset.y()
                ...
                {
                    double s_next = s + interval;
                    if (s_next > poRing->get_Length())
                        s_next = poRing->get_Length();
                    ...
                    point = point_next;
                    // std::cout << "Boundary trace: s: " << s << ", x: " << x << ", y:"  << y << ", tile min: (" << tileXMin << ", " << tileYMin <<
                    // "), max: (" << tileXMax << ", " << tileYMax << ")" << std::endl;
                    ...
                    if (tileBB.contains(osg::Vec3(x, y, 0.0)))
                    {
                        ...
                        if (x_int < 0)
                            x_int = 0;
                        else if (x_int >= field->getNumColumns())
                            x_int = field->getNumColumns() - 1;
                        int y_int = floor((y - tileYMin) / tileYInterval + 0.5);
                        if (y_int < 0)
                            y_int = 0;
                        else if (y_int >= field->getNumRows())
                            y_int = field->getNumRows() - 1;
    ...
    if (colorLayer)
    {
        ...
        fftw_plan fftwForward1 = fftw_plan_dft_2d(inputImage->s(),
                                                  inputImage->t(),
                                                  reinterpret_cast<fftw_complex*>(fftwImag1),
                                                  reinterpret_cast<fftw_complex*>(fftwFreq1),
                                                  FFTW_FORWARD,
                                                  FFTW_ESTIMATE);
        fftw_plan fftwForward2 = fftw_plan_dft_2d(inputImage->s(),
                                                  inputImage->t(),
                                                  reinterpret_cast<fftw_complex*>(fftwImag2),
                                                  reinterpret_cast<fftw_complex*>(fftwFreq2),
                                                  FFTW_FORWARD,
                                                  FFTW_ESTIMATE);

        fftw_plan fftwBackward1 = fftw_plan_dft_2d(inputImage->s(),
                                                   inputImage->t(),
                                                   reinterpret_cast<fftw_complex*>(fftwFreq1),
                                                   reinterpret_cast<fftw_complex*>(fftwImag1),
                                                   FFTW_BACKWARD,
                                                   FFTW_ESTIMATE);
        fftw_plan fftwBackward2 = fftw_plan_dft_2d(inputImage->s(),
                                                   inputImage->t(),
                                                   reinterpret_cast<fftw_complex*>(fftwFreq2),
                                                   reinterpret_cast<fftw_complex*>(fftwImag2),
                                                   FFTW_BACKWARD,
        ...
        for (int x = 0; x < inputImage->s(); ++x)
        {
            ...
            {
                if (inputImage->getPixelFormat() == 0x83f0)
                {  // COMPRESSED_RGB_S3TC_DXT1_EXT decompression
                    ...
                    (*(fftwImag2 + inputImage->t() * x + y)) =
                        ((double)g * (double)0xff / (double)0x3f * e3 * e1) * e3 * e1 + ((double)b * (double)0xff / (double)0x1f * e1 * e2);
        ...
        for (int x = 0; x < inputImage->s(); ++x)
        {
            for (int y = 0; y < inputImage->t(); ++y)
            {
                ...
                if (x <= inputImage->s() / 2)
                    ...
                    s = -(double)(inputImage->s() - x);
                if (y <= inputImage->t() / 2)
                    t = (double)y;
                else
                    t = -(double)(inputImage->t() - y);
        ...
        /*for(int x=0; x<inputImage->s(); ++x) {
           for(int y=0; y<inputImage->t(); ++y) {
              ...
              //   std::cout << "double: " << r_double << ", " << g_double << ", " << b_double << ", int: " << (int)r << ", " << (int)g << ", " <<
        (int)b << std::endl;
              //}
           }
        }*/
        ...
        for (int i = 0; i < nTrees; ++i)
        {
            ...
            unsigned char r =
                (unsigned char)((double)eval(part_type<em::mv<6>::type>(*(fftwImag1 + inputImage->t() * img_x + img_y)) * e3 * e2) * invSize);
            unsigned char g =
                (unsigned char)((double)eval((part_type<em::mv<0>::type>(*(fftwImag2 + inputImage->t() * img_x + img_y)) * e1 * e3) * e1 * e3) *
                                invSize);
            unsigned char b =
                (unsigned char)((double)eval(part_type<em::mv<3>::type>(*(fftwImag2 + inputImage->t() * img_x + img_y)) * e2 * e1) * invSize);
            if (r > 200)
            {
                bool found = false;
                ...
                if (x_int < 0)
                    x_int = 0;
                else if (x_int >= (int)field->getNumColumns())
                    x_int = field->getNumColumns() - 1;
                int y_int = floor((y - tileYMin) / tileYInterval + 0.5);
                if (y_int < 0)
                    y_int = 0;
                else if ((unsigned int)y_int >= field->getNumRows())
                    y_int = field->getNumRows() - 1;
                double z = field->getHeight(x_int, y_int);

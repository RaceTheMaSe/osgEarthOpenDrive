...
void DecoratedGeometryTechnique::generateGeometry(BufferData &buffer, osgTerrain::Locator *masterLocator, const osg::Vec3d &centerModel)
{
    ...
    for (j = 0; j < numRows; ++j)
    {
        for (i = 0; i < numColumns; ++i)
        {
            ...
            if (elevationLayer)
            {
                ...
                if (fillBorderMapIt != fillBorderMap.end())
                {
                    ...
                    if (xzMapIt != fillBorderMapIt->second.end())
                    {
                        ...
                        // validValue = false;
                        std::cout << "Changing height in tile L_" << tileId.level << "_X" << tileId.x << "_Y" << tileId.y << ": x=" << i_equiv
                                  << ", y=" << j_equiv << " to z=" << ndc.z() << std::endl;
                    }
                }
            }
            ...
            for (int rectIt = 0; rectIt < voidBoundingAreaVector.size(); ++rectIt)
            {
                ...
                {
                    // std::cout << "Checking rect " << rectIt << ": (" << voidBoundingAreaVector[rectIt]._min.x() << ", " <<
                    // voidBoundingAreaVector[rectIt]._min.y() << ") - (" <<  voidBoundingAreaVector[rectIt]._max.x() << ", " <<
                    // voidBoundingAreaVector[rectIt]._max.y() << ") for (" << model.x() << ", " << model.y() << ")" << std::endl;
                    validValue = false;
                    break;
                }
            }
            ...
            if (validValue)
            {
                ...
                for (LayerToTexCoordMap::iterator itr = layerToTexCoordMap.begin(); itr != layerToTexCoordMap.end(); ++itr)
                {
    ...
    osg::ref_ptr<osg::DrawElements> elements = smallTile ? static_cast<osg::DrawElements *>(new osg::DrawElementsUShort(GL_TRIANGLES))
    ...
    if (createSkirt)
    {
        osg::ref_ptr<osg::DrawElements> skirtDrawElements = smallTile ? static_cast<osg::DrawElements *>(new osg::DrawElementsUShort(GL_QUAD_STRIP))
        ...
        for (c = 0; c < static_cast<int>(numColumns); ++c)
        {
            ...
            else
            {
                if (skirtDrawElements->getNumIndices() != 0)
                {
                    ...
                    skirtDrawElements = smallTile ? static_cast<osg::DrawElements *>(new osg::DrawElementsUShort(GL_QUAD_STRIP))
                                                  : static_cast<osg::DrawElements *>(new osg::DrawElementsUInt(GL_QUAD_STRIP));
                }
            }
        }
        ...
        {
            geometry->addPrimitiveSet(skirtDrawElements.get());
            smallTile ? static_cast<osg::DrawElements *>(new osg::DrawElementsUShort(GL_QUAD_STRIP))
                      : static_cast<osg::DrawElements *>(new osg::DrawElementsUInt(GL_QUAD_STRIP));
        }
    }
    ...
    if (_terrainTile->getTerrain())
    {
       ...
       if(treeStateSet) {
        ...
        // std::cout << "Master tile: name: " << masterTile->getName() << ", coordinate system: " << masterTile->getLocator()->getCoordinateSystem()
        // << std::endl;
                ...
                if (shapeGeometry)
                {
                    ...
                    if (hasNameForest /*&& objNameInList*/)
                    {
                        ...
                        if (std::string(shapeGeometry->getVertexArray()->className()) == "Vec3Array")
                        {
                            ...
                            if (((b_max.x() >= a_min.x() && b_max.x() <= a_max.x()) || (b_min.x() >= a_min.x() && b_min.x() <= a_max.x()) ||
                                ...
                                (b_max.y() >= a_min.y() && b_min.y() <= a_max.y()))
                            {
                                ...
                                for (double scanline = lower_left.y() + 5.0; scanline <= upper_right.y() - 5.0; scanline += 10.0, ++scanlineIt)
                                {
                                    ...
                                    for (unsigned int treeIt = 0; treeIt < vertexArray->getNumElements() - 2; treeIt += 2)
                                    {
                                        ...
                                        if (t > 0.0 && t < 1.0)
                                        {
                                            // scanLineMap[scanline].insert(std::make_pair(pos0.x() + t*(pos1.x()-pos0.x()), pos0.z() +
                                            // t*(pos1.z()-pos0.z())));
                                ...
                                if (scanLineMap.begin() != scanLineMap.end())
                                {
                                    ...
                                    for (std::map<double, std::set<double> >::iterator scanLineIt = scanLineMap.begin();
                                         scanLineIt != scanLineMap.end();
                                         ++scanLineIt)
                                    {
                                        ...
                                        for (std::set<double>::iterator cutIt = scanLineIt->second.begin(); cutIt != scanLineIt->second.end();
                                             ++cutIt, ++cutIt)
                                        {
                                            ...
                                            for (double x = x0 + 5.0; x < x1 - 5.0; x += 10.0, ++xIt)
                                            {
                                                ...
                                                if (lower_left_tile.y() <= y && upper_right_tile.y() >= y && lower_left_tile.x() <= x &&
                                                    upper_right_tile.x() >= x)
                                                {
                                                    ...
                                                    // std::cout << "Adding tree at (" << treePos.x() << ", " << treePos.y() << ", " << treePos.z() <<
                                                    // std::endl;
                        ...
                        if (std::string(shapeGeometry->getVertexArray()->className()) == "Vec3Array")
                        {
                         ...
                         && lower_left_tile.x() <= buildingPos1.x() && upper_right_tile.x() >= buildingPos1.x()*/)
                            ...
                            if (!buildingOutsideTile && buildingGroundVerts.size() > 1)
                            {
                                ...
                                osg::ref_ptr<osg::DrawArrays> buildingDrawArrays =
                                    new osg::DrawArrays(osg::PrimitiveSet::QUAD_STRIP, buildingStartVertIt, buildingVertIt - buildingStartVertIt);

...
RoadTerrainLoader::RoadTerrainLoader() : terrainLoaded(false), layerLoaded(false)
{
...
bool RoadTerrainLoader::loadTerrain(std::string                      filename,
                                    osg::Vec3d                       offset,
                                    ...
                                    const std::vector<std::string>  &shapeFileNameVector)
{
    ...
    if (coop)
    {
        for (int screenIt = 0; screenIt < 1 /*coVRConfig::instance()->numScreens()*/; ++screenIt)
        {
            osgViewer::Renderer *renderer = nullptr;  // new osgViewer::Renderer;
            if (renderer)
            {
        ...
        std::cout << "\t getMinimumTimeAvailableForGLCompileAndDeletePerFrame: " << coop->getMinimumTimeAvailableForGLCompileAndDeletePerFrame()
                  << std::endl;
    ...
    if (terrain)
    {
        ...
        terrainStateSet = terrain->getOrCreateStateSet();
        ...
        osg::Vec3d offsetVec = {};
        osgTerrain::TerrainTile::setTileLoadedCallback(new LevelTerrainCallback(-offsetVec, voidBoundingAreaVector, shapeFileVector));
        ...
        terrainTransform->setPosition(-offsetVec);

        // cover->getObjectsRoot()->addChild(terrainTransform);

        const osg::BoundingSphere &terrainBS = terrain->getBound();
        std::cout << "Terrain BB: center: (" << terrainBS.center()[0] << ", " << terrainBS.center()[1] << ", " << terrainBS.center()[2]
                  << "), radius: " << terrainBS.radius() << std::endl;

...
RoadObject::RoadObject(const std::string &setId,
                       const std::string &setFile,
                       const std::string &setTextureFile,
                       const std::string &setName,
                       const std::string &setType,
                       const double      &setS,
                       const double      &setT,
                       const double      &setZOffset,
                       const double      &setValidLength,
                       OrientationType    setOrientation,
                       const double      &setLength,
                       const double      &setWidth,
                       const double      &setRadius,
                       const double      &setHeight,
                       const double      &setHdg,
                       const double      &setPitch,
                       const double      &setRoll,
                       Road              *roadP)
    : Element(setId),
      fileName(setFile),
...
osg::Node *RoadObject::loadObjectGeometry(std::string file)
{
    ...
    else if (type == "guardRail" || type == "barrier")
    {
        absolute = true;
    ...
    else if (type == "reflectorPost" || type == "pole")
    {
        absolute = false;
        if (reflectorPostGeode == NULL)
        {
            reflectorPostGeode = createReflectorPostGeode("reflectorPost");
    ...
    else if (type == "reflectorPostYellow")
    {
        ...
        {
            reflectorPostYellowGeode = createReflectorPostGeode("reflectorPostYellow");
    ...
    else
    {
        ...
        osg::Group *objectGroup     = new osg::Group();  // objectGroup is argument for coVRFileManager::instance()->loadFile() so that the loaded
                                                         // osg::Node isn't hooked to the opencover root node automatically...
        ...
        else
        {
            ...
            {
                std::string nameDisp = name + " - " + type + " (" + file + ")";
                std::cout << "RoadObject::getObjectGeometry(): Couldn't load: \"" << nameDisp << "\"..." << std::endl;
                osg::ShapeDrawable* fake = new osg::ShapeDrawable(new osg::Cylinder({} /* center */,
                                                            0.2 /* radius */,
                                                            0.8 /* height */),
                                                            new osg::TessellationHints());
                fake->setName(nameDisp);
                localObjectNode = fake;
                return fake;
            }
...
osg::Geode *RoadObject::createReflectorPostGeode(const std::string &textureName)
{
...
osg::Geode *RoadObject::createGuardRailGeode()
{
    ...
    for (double currentS = s; currentS <= s + railLength; currentS += h)
    {
        ...
        if (road->getLaneSection(currentS) != currentLaneSection)
        {
            ...
            if (t < 0)
            {
                if (fabs(currentT) < currentLaneSection->getLaneSpanWidth(0, currentLaneId + 1, currentS) +
                                         currentLaneSection->getLaneWidth(currentS, currentLaneId) / 2)
                {
                    currentLaneId++;
                }
            ...
            {
                if (currentT < currentLaneSection->getLaneSpanWidth(0, currentLaneId - 1, currentS) +
                                   currentLaneSection->getLaneWidth(currentS, currentLaneId) / 2)
                {
                    currentLaneId--;
                }
        ...
        if (up || down)
        {
            guardRailVertices->push_back(ConvertToMapCoordinatesFloat(osg::Vec3d(railPoint.x(), railPoint.y(), railPoint.z() - 0.31)));
            guardRailTexCoords->push_back(osg::Vec2((currentS / texlength), 0));

            guardRailVertices->push_back(ConvertToMapCoordinatesFloat(osg::Vec3d(railPoint.x(), railPoint.y(), railPoint.z())));
            guardRailTexCoords->push_back(osg::Vec2((currentS / texlength), 0.33 / texwidth));
        }
        else
        {
            guardRailVertices->push_back(ConvertToMapCoordinatesFloat(osg::Vec3d(railPoint.x(), railPoint.y(), railPoint.z() + 0.44)));
            guardRailTexCoords->push_back(osg::Vec2((currentS / texlength), 0));

            guardRailVertices->push_back(ConvertToMapCoordinatesFloat(osg::Vec3d(railPoint.x(), railPoint.y(), railPoint.z() + 0.75)));
            guardRailTexCoords->push_back(osg::Vec2((currentS / texlength), 0.33 / texwidth));
            ...
            guardRailPostVertices->push_back(ConvertToMapCoordinatesFloat(osg::Vec3d(p1)));
            guardRailPostTexCoords->push_back(osg::Vec2(0.02, 0.02));
            guardRailPostNormals->push_back(n);
            guardRailPostVertices->push_back(ConvertToMapCoordinatesFloat(osg::Vec3d(p2)));
            guardRailPostTexCoords->push_back(osg::Vec2(0.02, 0.1));
            guardRailPostNormals->push_back(n);
            guardRailPostVertices->push_back(ConvertToMapCoordinatesFloat(osg::Vec3d(p6)));
            guardRailPostTexCoords->push_back(osg::Vec2(0.1, 0.1));
            guardRailPostNormals->push_back(n);
            guardRailPostVertices->push_back(ConvertToMapCoordinatesFloat(osg::Vec3d(p5)));
            ...
            guardRailPostVertices->push_back(ConvertToMapCoordinatesFloat(osg::Vec3d(p3)));
            guardRailPostTexCoords->push_back(osg::Vec2(0.02, 0.1));
            guardRailPostNormals->push_back(negn3);
            guardRailPostVertices->push_back(ConvertToMapCoordinatesFloat(osg::Vec3d(p7)));
            guardRailPostTexCoords->push_back(osg::Vec2(0.1, 0.1));
            guardRailPostNormals->push_back(negn3);
            guardRailPostVertices->push_back(ConvertToMapCoordinatesFloat(osg::Vec3d(p6)));
            guardRailPostTexCoords->push_back(osg::Vec2(0.1, 0.02));
            ...
            guardRailPostTexCoords->push_back(osg::Vec2(0.02, 0.02));
            guardRailPostNormals->push_back(negn3);
            guardRailPostVertices->push_back(ConvertToMapCoordinatesFloat(osg::Vec3d(p4)));
            guardRailPostTexCoords->push_back(osg::Vec2(0.02, 0.1));
            guardRailPostNormals->push_back(negn3);
            guardRailPostVertices->push_back(ConvertToMapCoordinatesFloat(osg::Vec3d(p8)));
            guardRailPostTexCoords->push_back(osg::Vec2(0.1, 0.1));
            guardRailPostNormals->push_back(negn3);
            guardRailPostVertices->push_back(ConvertToMapCoordinatesFloat(osg::Vec3d(p5)));
            guardRailPostTexCoords->push_back(osg::Vec2(0.1, 0.02));
...
osg::Geode *RoadObject::createSTEPBarrierGeode()
{
    ...
    for (int pseg = 0; pseg < numSegments; pseg++)
    {
        for (double currentS = s; currentS <= s + railLength; currentS += h)
        {
            ...
            STEPBarrierVertices->push_back(ConvertToMapCoordinatesFloat(osg::Vec3d(railPoint.x(), railPoint.y(), railPoint.z() + 0.44)));
            STEPBarrierTexCoords->push_back(osg::Vec2((currentS / texlength), 0));

            STEPBarrierVertices->push_back(ConvertToMapCoordinatesFloat(osg::Vec3d(railPoint.x(), railPoint.y(), railPoint.z() + 0.75)));
        ...
        for (double currentS = s; currentS <= s + railLength; currentS += h)
        {
            ...
            if (road->getLaneSection(currentS) != currentLaneSection)
            {
                ...
                while (currentLaneSection && (currentLaneSection != newLaneSection))
                {
                    ...
                    {
                        currentT =
                            -currentLaneSection->getLaneSpanWidth(0, currentLaneId, road->getLaneSectionEnd(currentLaneSection->getStart())) + d;
                    }
                    else if (t > 0)
                    {
                        currentT =
            ...
            p1 += n * profile[pseg].x();
            p1[2] += profile[pseg].y();
            OutlineVertices->push_back(ConvertToMapCoordinatesFloat(osg::Vec3d(p1.x(), p1.y(), p1.z())));
            ...
            OutlineVertices->push_back(ConvertToMapCoordinatesFloat(osg::Vec3d(p2.x(), p2.y(), p2.z())));

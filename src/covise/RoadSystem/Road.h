...
namespace vehicleUtil
{
    ...
    public:
        ...
        osg::Group* getRoadGroup(bool tessellateBatters, bool tessellateObjects);
        ...
        std::vector<osg::Vec3dArray*> getRoadOutline();
        Vector2D getOrQueryElevation(double s, const Vector3D& xyPoint);
    ...
    protected:
        ...
        osg::Group* roadGroup;
        std::vector<osg::Vec3dArray*> roadOutline;
    ...
    {
    public:
        RoadTransition(Road* r, int d, Junction* j = NULL) : road(r), direction(d), junction(j)
        {
        }
        RoadTransition() : road(NULL), direction(0), junction(NULL)
        {
        }
        ...
        bool operator<(const RoadTransition& trans) const
        {
            return (road->getHeading(direction == -1 ? road->getLength() : 0.0) <
                    trans.road->getHeading(trans.direction == -1 ? trans.road->getLength() : 0.0));
        }
    };
}  // namespace vehicleUtil

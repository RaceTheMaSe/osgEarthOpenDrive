...
Road::Road(std::string id, std::string name, double l, Junction *junc) : Tarmac(id, name)
{
    shapeSections = NULL;
    length        = l;
    junction      = junc;
...
// query external elevation data if map has non set
Vector2D Road::getOrQueryElevation(double s, const Vector3D& xyPoint)
{
    if(zMap.size()==1 )
    {
        double a,b,c,d;
        Polynom& poly = *zMap.begin()->second;
        poly.getCoefficients(a,b,c,d);
        if(a==0.0 && b==0.0 && c==0.0 && d==0.0)
            return Vector2D(sampleElevation(osg::Vec3d(xyPoint.x(),xyPoint.y(),0.0)),0.0);
    }
    return ((--zMap.upper_bound(s))->second)->getPoint(s);
}
...
void Road::addPlanViewGeometryPolynom(double s,
                                      double l,
                                      double x,
                                      double y,
                                      double hdg,
...
osg::Group* Road::getSignalsGroup()
{
    osg::Group* sg = new osg::Group;
    roadGroup->addChild(sg);
    for(auto& s : roadSignalVector)
    {
        // transform here instead of on create as it might be called before mapConversion info is processed and initialized
        Transform tf = s->getTransform();
        osg::Vec3 pos = ConvertToMapCoordinatesFloat(osg::Vec3d(tf.v().x(), tf.v().y(), tf.v().z()));
        tf.v() = Vector3D(pos.x(), pos.y(), pos.z());
        s->setTransform(tf);
        osg::PositionAttitudeTransform* patS = s->getRoadSignalNode();
        osg::PositionAttitudeTransform* patP = s->getRoadSignalPost();
        if(patS)
            sg->addChild(patS);
        if(patP)
            sg->addChild(patP);
...
Vector3D Road::getCenterLinePoint(double s)
{
    ...
    Vector2D zPoint  = getOrQueryElevation(s,xyPoint);

    return Vector3D(xyPoint.x(), xyPoint.y(), zPoint[0]);
}

Vector3D Road::getTangentVector(double s)
{
    return Vector3D(((--xyMap.upper_bound(s))->second)->getTangentVector(s), ((--zMap.upper_bound(s))->second)->getSlope(s));
}

Vector3D Road::getNormalVector(double s)
{
    return Vector3D(((--xyMap.upper_bound(s))->second)->getNormalVector(s), ((--zMap.upper_bound(s))->second)->getCurvature(s));
}

RoadPoint Road::getChordLinePoint(double s)
{
    Vector3D xyPoint(((--xyMap.upper_bound(s))->second)->getPoint(s));
    double   zValue = getOrQueryElevation(s,xyPoint).x();
...
{
    Vector3D xyPoint(((--xyMap.upper_bound(s))->second)->getPoint(s));
    return getOrQueryElevation(s,xyPoint).x();
}

double Road::getChordLineElevationSlope(double s)
{
    // FIXME: derive from multiple elevation samples
...
void Road::getLaneRoadPoints(double s, int i, RoadPoint &pointIn, RoadPoint &pointOut, double &disIn, double &disOut)
{
    ...
    // std::cout << "s: " << s << ", i: " << i << ", outer: x: " << pointOut.x() << ", y: " << pointOut.y() << ", z: " << pointOut.z() << std::endl <<
    // std::endl;
}
...
void Road::getBatterPoint(double s, int i, RoadPoint &pointOut, RoadPoint &pointCenter, RoadPoint &pointIn, double &width, int marker)
{
    ...
    // pointIn = pointOut;
    // pointCenter = RoadPoint ( (pointOut.x() -
    // batterpointIn.x())/(sqrt((pointOut.x()-batterpointIn.x())*(pointOut.x()-batterpointIn.x())+(pointOut.y()-batterpointIn.y())*(pointOut.y()-batterpointIn.y())+(pointOut.z()-batterpointIn.z())*(pointOut.z()-batterpointIn.z())))+pointOut.x()
    // , (pointOut.y() -
    // batterpointIn.y())/(sqrt((pointOut.x()-batterpointIn.x())*(pointOut.x()-batterpointIn.x())+(pointOut.y()-batterpointIn.y())*(pointOut.y()-batterpointIn.y())+(pointOut.z()-batterpointIn.z())*(pointOut.z()-batterpointIn.z())))+pointOut.y(),
    // (pointOut.z() -
    // batterpointIn.z())/(sqrt((pointOut.x()-batterpointIn.x())*(pointOut.x()-batterpointIn.x())+(pointOut.y()-batterpointIn.y())*(pointOut.y()-batterpointIn.y())+(pointOut.z()-batterpointIn.z())*(pointOut.z()-batterpointIn.z())))+pointOut.z(),
    // pointOut.nx(), pointOut.ny(), pointOut.nz());
    //
    double disIn, disOut;
    ...
    pointCenter           = RoadPoint(pointIn.x() + bitangent_sign * bitangent.x(),
                            pointIn.y() + bitangent_sign * bitangent.y(),
                            pointIn.z() + bitangent_sign * bitangent.z(),
                            pointIn.nx(),
                            pointIn.ny(),
                            pointIn.nz());
    ...
    // pointOut = RoadPoint ( width*(pointCenter.x() - batterpointOut.x())+pointCenter.x() , width*(pointCenter.y()-batterpointOut.y()) +
    // pointCenter.y() ,(width*(pointCenter.z()-batterpointOut.z()) + pointCenter.z()) - (sin(batterFall)*width), batterpointOut.nx(),
    // batterpointOut.ny(), batterpointOut.nz()); pointOut = RoadPoint ( width*(bitangent_sign*bitangent.x())+pointCenter.x() ,
    // width*(bitangent_sign*bitangent.y()) + pointCenter.y() ,(width*(bitangent_sign*bitangent.z()) + pointCenter.z()) - (sin(batterFall)*width),
    // pointIn.nx(), pointIn.ny(), pointIn.nz());
...
void Road::getGuardRailPoint(RoadPoint                                &pointDown,
                             RoadPoint                                &pointUp,
                             bool                                      firstRound,
                             bool                                      lastRound,
                             std::map<double, LaneSection *>::iterator lsIt)
{
    ...
    if (firstRound == true)
    {
        ...
        else
        {
            ...
            if(previouslsIt != laneSectionMap.end())
                --previouslsIt;
    ...
    e = RoadPoint(
        (sqrt((pointDown.x() - pointUp.x()) * (pointDown.x() - pointUp.x()) + (pointDown.y() - pointUp.y()) * (pointDown.y() - pointUp.y()) +
              (pointDown.z() - pointUp.z()) * (pointDown.z() - pointUp.z()))),
        (sqrt((pointDown.x() - pointUp.x()) * (pointDown.x() - pointUp.x()) + (pointDown.y() - pointUp.y()) * (pointDown.y() - pointUp.y()) +
              (pointDown.z() - pointUp.z()) * (pointDown.z() - pointUp.z()))),
        (sqrt((pointDown.x() - pointUp.x()) * (pointDown.x() - pointUp.x()) + (pointDown.y() - pointUp.y()) * (pointDown.y() - pointUp.y()) +
              (pointDown.z() - pointUp.z()) * (pointDown.z() - pointUp.z()))));
    ...
    double xAdd = e.x() == 0.0 ? 0.0 : ((pointDown.x() - pointUp.x()) / e.x());
    double yAdd = e.y() == 0.0 ? 0.0 : ((pointDown.y() - pointUp.y()) / e.y());
    double zAdd = e.z() == 0.0 ? 0.0 : ((pointDown.z() - pointUp.z()) / e.z());
    
    pointDown = RoadPoint(0.5 * xAdd + pointDown.x(),
                          0.5 * yAdd + pointDown.y(),
                          0.5 * zAdd + pointDown.z() + (0.44 - width),
                          pointUp.x() - pointDown.x(),
                          pointUp.y() - pointDown.y(),
                          pointUp.z() - pointDown.z());
...
osg::Geometry *Road::getGuardRailPost(RoadPoint pointDown, RoadPoint pointUp, RoadPoint postPoint, RoadPoint nextrailPointDown) // could be susbstituted by a cube geometry / instanced object with per object vertex array, sharing indices and uvs if this is possible / supported by the implementation
{
    ...
    p1Down = RoadPoint(0.51 * xAdd + pointDown.x(),
                       0.51 * yAdd + pointDown.y(),
                       0.51 * zAdd + pointDown.z());

    p2Down = RoadPoint(0.7 * xAdd + pointDown.x(),
                       0.7 * yAdd + pointDown.y(),
                       0.7 * zAdd + pointDown.z(),
                       pointUp.x() - pointDown.x(),
                       pointUp.y() - pointDown.y(),
                       pointUp.z() - pointDown.z());

    p3Down = p3Down = RoadPoint(0.04 * (postPoint.x() - nextrailPointDown.x()) + p2Down.x(),
                                0.04 * (postPoint.y() - nextrailPointDown.y()) + p2Down.y(),
                                0.04 * (postPoint.z() - nextrailPointDown.z()) + p2Down.z());

    p4Down = RoadPoint(0.04 * (postPoint.x() - nextrailPointDown.x()) + p1Down.x(),
                       0.04 * (postPoint.y() - nextrailPointDown.y()) + p1Down.y(),
                       0.04 * (postPoint.z() - nextrailPointDown.z()) + p1Down.z());
    ...
    guardRailPostVertices->push_back(ConvertToMapCoordinatesFloat(osg::Vec3d(p1Down.x(), p1Down.y(), p1Down.z())));
    guardRailPostTexCoords->push_back(osg::Vec2(0.0, 0.0));
    guardRailPostNormals->push_back(osg::Vec3(p4Down.x(), p4Down.y(), p4Down.z()));
    guardRailPostVertices->push_back(ConvertToMapCoordinatesFloat(osg::Vec3d(p2Down.x(), p2Down.y(), p2Down.z())));
    guardRailPostTexCoords->push_back(osg::Vec2(0.0, 1.0));
    guardRailPostNormals->push_back(osg::Vec3(p4Down.x(), p4Down.y(), p4Down.z()));
    guardRailPostVertices->push_back(ConvertToMapCoordinatesFloat(osg::Vec3d(p2Up.x(), p2Up.y(), p2Up.z())));
    guardRailPostTexCoords->push_back(osg::Vec2(1.0, 1.0));
    guardRailPostNormals->push_back(osg::Vec3(p4Down.x(), p4Down.y(), p4Down.z()));
    guardRailPostVertices->push_back(ConvertToMapCoordinatesFloat(osg::Vec3d(p1Up.x(), p1Up.y(), p1Up.z())));
    ...
    guardRailPostVertices->push_back(ConvertToMapCoordinatesFloat(osg::Vec3d(p3Down.x(), p3Down.y(), p3Down.z())));
    guardRailPostTexCoords->push_back(osg::Vec2(1.0, 0.0));
    guardRailPostNormals->push_back(osg::Vec3(p1Down.x(), p1Down.y(), p1Down.z()));
    guardRailPostVertices->push_back(ConvertToMapCoordinatesFloat(osg::Vec3d(p3Up.x(), p3Up.y(), p3Up.z())));
    guardRailPostTexCoords->push_back(osg::Vec2(1.0, 1.0));
    guardRailPostNormals->push_back(osg::Vec3(p1Down.x(), p1Down.y(), p1Down.z()));
    guardRailPostVertices->push_back(ConvertToMapCoordinatesFloat(osg::Vec3d(p2Up.x(), p2Up.y(), p2Up.z())));
    ...
    guardRailPostTexCoords->push_back(osg::Vec2(1.0, 1.0));
    guardRailPostNormals->push_back((-osg::Vec3(p4Down.x(), p4Down.y(), p4Down.z())));
    guardRailPostVertices->push_back(ConvertToMapCoordinatesFloat(osg::Vec3d(p4Down.x(), p4Down.y(), p4Down.z())));
    guardRailPostTexCoords->push_back(osg::Vec2(0.0, 1.0));
    guardRailPostNormals->push_back(-(osg::Vec3(p4Down.x(), p4Down.y(), p4Down.z())));
    guardRailPostVertices->push_back(ConvertToMapCoordinatesFloat(osg::Vec3d(p4Up.x(), p4Up.y(), p4Up.z())));
    guardRailPostTexCoords->push_back(osg::Vec2(1.0, 0.0));
    guardRailPostNormals->push_back(-(osg::Vec3(p4Down.x(), p4Down.y(), p4Down.z())));
    guardRailPostVertices->push_back(ConvertToMapCoordinatesFloat(osg::Vec3d(p3Up.x(), p3Up.y(), p3Up.z())));
    guardRailPostTexCoords->push_back(osg::Vec2(0.0, 0.0));
...
Vector2D Road::searchPositionNoBorder(const Vector3D &pos, double sinit)
{
    ...
    if(area < minArea)
    {
            ...
            /*double circum = supportVectorVector[i].length() + supportVectorVector[i+1].length() + (supportVectorVector[i] -
            supportVectorVector[i+1]).length(); if(circum < minCircum)
            {
                    minCircum = circum;
                    minPointer = i;
            }*/
        ...
        // std::cout << "Choosing " << minPointer+1 << ". road piece: initial s: " << s << ", ndist: " << minDistance << ", tgamma: " << minGamma <<
        // std::endl;
    }
    else
    {
    ...
    do
    {
        ...
        {
            // std::cout << "Lost position on road " << getId() << ", s: " << s << ", iterations: " << it << ", pos: " << pos << ", sinit: " << sinit
...
osg::Geode *Road::createGuardRailGeode(std::map<double, LaneSection *>::iterator lsIt)
{
    ...
    for (double s = lsStart; s < (lsEnd); s += h)
    {
        ...
        if (firstRound)
        {
            ...
            guardRailVertices->push_back(ConvertToMapCoordinatesFloat(osg::Vec3d(railPointDown.x(), railPointDown.y(), railPointDown.z())));
            guardRailTexCoords->push_back(osg::Vec2((s / texlength), 0));

            guardRailVertices->push_back(ConvertToMapCoordinatesFloat(osg::Vec3d(railPointUp.x(), railPointUp.y(), railPointUp.z())));
        ...
        v1 = ConvertToMapCoordinatesFloat(osg::Vec3d(nextrailPointUp.x() - railPointUp.x(), nextrailPointUp.y() - railPointUp.y(), nextrailPointUp.z() - railPointUp.z()));
        v2 = ConvertToMapCoordinatesFloat(osg::Vec3d(railPointDown.x() - railPointUp.x(), railPointDown.y() - railPointUp.y(), railPointDown.z() - railPointUp.z()));
        ...
        {
            /*
                osg::Box* post = new osg::Box(osg::Vec3(1.22*(nextrailPointDown.x() - postPoint.x())+postPoint.x(),1.22*(nextrailPointDown.y() -
                postPoint.y())+postPoint.y(),1.22*((nextrailPointDown.z()-0.44) - postPoint.z())+postPoint.z()+0.25),0.2,0.1,0.7); osg::Quat quat;
    ...
    if(roadGroup)
        roadGroup->addChild(guardRailGeode);
...
osg::Group *Road::getRoadGroup(bool tessellateBatters, bool tessellateObjects)
{
    if (roadGroup == nullptr)
    {
        createRoadGroup(tessellateBatters,tessellateObjects);
        AddNodeWithLod(roadGroup,createObjectsGroup());
        AddNodeWithLod(roadGroup,getSignalsGroup());
    }
    return roadGroup;
}
...
osg::Group *Road::createRoadGroup(bool tessellateBatters, bool tessellateObjects)
{
    ...
    AddNodeWithLod(roadGroup,roadGeode);
    ...
    // outline addition:
    std::vector<RoadPoint>        roadEdgeLeft = {};
    std::vector<RoadPoint>        roadEdgeRight = {};
    std::vector<RoadPoint>        roadStartCap = {};
    std::vector<RoadPoint>        roadEndCap = {};
    std::vector<osg::Vec3dArray*> laneSectionOutlines;
    ...
    for (lsIt = laneSectionMap.begin(); lsIt != laneSectionMap.end(); ++lsIt)
    {
        ...
        if (lsStart > length)
        {
            std::cerr << "Inconsistency: Lane Section defined with start position " << lsStart << " not in road boundaries 0 - " << lsEnd << "!"
                      << std::endl;
            break;
        }
        ...
        for (int i = (ls->getNumLanesLeft()); i >= -(ls->getNumLanesRight()); --i)
        {
            ...
            bool leftOutline  = i ==  ls->getNumLanesLeft();
            bool rightOutline = i == -ls->getNumLanesRight();
            ...
            for (double s = lsStart; s < (lsEnd + h); s += h)
            {
                ...
                if ((curvatureFactor * h / (s - lastTess) < fabs(getCurvature(s))) || (curvatureFactor * h / (s - lastTess) < fabs(getChordLineElevationCurvature(s))) ||
                    lastWidth != width || lastWidthOffset != widthOffset || lastRound || firstRound)
                {
                    ...
                    // std::cerr << "s: " << s << ", i: " << "Outer mark width: " << markOut->getWidth() << ", inner mark width: " <<
                    // markIn->getWidth() << std::endl; std::cerr << "s: " << s << ", i: " << i << ", width: " << width << std::endl; std::cout << "s:
                    // " << s << ", i: " << i << ", left Point: x: " << pointLeft.x() << ", y: " << pointLeft.y() << ", z: " << pointLeft.z() <<
                    // std::endl; std::cerr << ", outer Point: x: " << pointOut.x() << ", y: " << pointOut.y() << ", z: " << pointOut.z() <<
                    // std::endl; std::cout << "s: " << s << ", i: " << i << ", inner Point: x: " << pointIn.nx() << ", y: " << pointIn.ny() << ", z:
                    // " << pointIn.nz(); std::cout << ", outer Point: x: " << pointOut.nx() << ", y: " << pointOut.ny() << ", z: " << pointOut.nz()
                    // << std::endl;

                    roadVertices->push_back(ConvertToMapCoordinates(osg::Vec3d(pointLeft.x(), pointLeft.y(), pointLeft.z())));
                    ...
                    roadMarks->push_back(osg::Vec4(-markLeft->getWidth() / width,
                                                   -markRight->getWidth() / width,
                                                   -markTypeLeft - (markLeft->getColor() * 1000.0),
                                                   -markTypeRight - (getPriority() * 100.0) - (markRight->getColor() * 1000.0)));
                    roadVertices->push_back(ConvertToMapCoordinates(osg::Vec3d(pointRight.x(), pointRight.y(), pointRight.z())));
                    ...
                    // lane section outlines addition
                    if(leftOutline)
                        roadEdgeLeft.push_back(pointLeft);
                    if(rightOutline)
                        roadEdgeRight.push_back(pointRight);
                    if(firstRound)
                    {
                        if(!leftOutline)
                            roadStartCap.push_back(pointLeft);
                    }
                    if(lastRound)
                    {
                        if(!leftOutline)
                            roadEndCap.push_back(pointLeft);
                    }
        ...
        // lane section outlines
        size_t numOfPoints = roadEdgeLeft.size() + roadEdgeRight.size() + roadStartCap.size() + roadEndCap.size();
        osg::Vec3dArray *roadOutlineVertices;
        roadOutlineVertices = new osg::Vec3dArray(numOfPoints);
        size_t i=0;
        for(auto it= roadEdgeRight.begin(); it != roadEdgeRight.end();++it)
            (*roadOutlineVertices)[i++].set((*it).x(),(*it).y(),(*it).z());
        for(auto it= roadEndCap.rbegin(); it != roadEndCap.rend();++it)
            (*roadOutlineVertices)[i++].set((*it).x(),(*it).y(),(*it).z());
        for(auto it= roadEdgeLeft.rbegin(); it != roadEdgeLeft.rend();++it)
            (*roadOutlineVertices)[i++].set((*it).x(),(*it).y(),(*it).z());
        for(auto it= roadStartCap.begin(); it != roadStartCap.end();++it)
            (*roadOutlineVertices)[i++].set((*it).x(),(*it).y(),(*it).z());
        laneSectionOutlines.push_back(roadOutlineVertices);
        roadEdgeLeft.clear();
        roadEdgeRight.clear();
        roadStartCap.clear();
        roadEndCap.clear();
        ...
        for (lsIt = laneSectionMap.begin(); lsIt != laneSectionMap.end(); ++lsIt)
        {
            ...
            for (std::list<int>::iterator batterIt = batterList.begin(); batterIt != batterList.end(); ++batterIt)
            {
                ...
                for (double s = lsStart; s < (lsEnd + h); s += h)
                {
                    ...
                    if ((curvatureFactor * h / (s - lastTess) < fabs(getCurvature(s))) ||
                        (curvatureFactor * h / (s - lastTess) < fabs(getChordLineElevationCurvature(s))) || lastRound || firstRound)
                    {
                        firstRound = false;
                        ...
                        firstbatterVertices->push_back(ConvertToMapCoordinatesFloat(osg::Vec3d(pointCenter.x(), pointCenter.y(), pointCenter.z())));
                        firstbatterNormals->push_back(osg::Vec3(pointCenter.nx(), pointCenter.ny(), pointCenter.nz()));

                        firstbatterVertices->push_back(ConvertToMapCoordinatesFloat(osg::Vec3d(pointRight.x(), pointRight.y(), pointRight.z())));
                        firstbatterNormals->push_back(osg::Vec3(pointRight.nx(), pointRight.ny(), pointRight.nz()));

                        secondbatterVertices->push_back(ConvertToMapCoordinatesFloat(osg::Vec3d(pointLeft.x(), pointLeft.y(), pointLeft.z())));
                        secondbatterNormals->push_back(osg::Vec3(pointLeft.nx(), pointLeft.ny(), pointLeft.nz()));

                        secondbatterVertices->push_back(ConvertToMapCoordinatesFloat(osg::Vec3d(pointCenter.x(), pointCenter.y(), pointCenter.z())));
    ...
    roadOutline = laneSectionOutlines;
...
osg::Group *Road::createObjectsGroup()
{
    ...
    for (std::vector<RoadObject *>::iterator objIt = roadObjectVector.begin(); objIt != roadObjectVector.end(); ++objIt)
    {
        ...
        else
        {
            ...
            osg::Quat objectAttitude(obj->getHdg() + ah,
                ...
                for (double s = obj->getS(); s < (obj->getS() + obj->getRepeatLength()); s += dist)
                {
                    if (getLaneSection(s) != currentLaneSection)
                    {
                        ...
                        while (currentLaneSection && (currentLaneSection != newLaneSection))
                        {
                            ...
                            {
                                currentT =
                                    -currentLaneSection->getLaneSpanWidth(0, currentLaneId, getLaneSectionEnd(currentLaneSection->getStart())) + d;
                            }
                            else if (obj->getT() > 0)
                            {
                                currentT =
                        ...
                        if (obj->getT() < 0)
                        {
                            if (fabs(currentT) < currentLaneSection->getLaneSpanWidth(0, currentLaneId + 1, s) +
                                                     currentLaneSection->getLaneWidth(s, currentLaneId) / 2)
                            {
                                currentLaneId++;
                            }
                        ...
                        {
                            if (currentT < currentLaneSection->getLaneSpanWidth(0, currentLaneId - 1, s) +
                                               currentLaneSection->getLaneWidth(s, currentLaneId) / 2)
                            {
                                currentLaneId--;
                            }
                    ...
                    objectTransform->setPosition(ConvertToMapCoordinatesFloat(osg::Vec3d(objectT.v().x(), objectT.v().y(), objectT.v().z() + obj->getZOffset())));
                    objectTransform->setAttitude(objectAttitude * osg::Quat(objectT.q().x(), objectT.q().y(), objectT.q().z(), objectT.q().w()) *
                                                 obj->getOrientation());
                }
            }
            else
...
std::vector<osg::Vec3dArray*> Road::getRoadOutline()
{
    return roadOutline;
}

...
TrafficLightSignalTurnCallback::TrafficLightSignalTurnCallback(osgSim::MultiSwitch *ms) : multiSwitch(ms)
{
}
...
RoadSignal::RoadSignal(const std::string     &setId,
                       const std::string     &setName,
                       const double          &setS,
                       const double          &setT,
                       const bool            &setDynamic,
                       const OrientationType &setOrient,
                       const double          &setZOffset,
                       const std::string     &setCountry,
                       const int             &setType,
                       const int             &setSubtype,
      ...
      height(setHeight)
{
    ...
    if (sp->signalNode.valid())
    {
        ...
        roadSignalNode->setAttitude(osg::Quat(signalTransform.q().x(), signalTransform.q().y(), signalTransform.q().z(), signalTransform.q().w()) *
                                    signalDir);
        ...
        if (sp->signalPost.valid())
        {
            ...
            roadSignalPost->setAttitude(
                osg::Quat(signalTransform.q().x(), signalTransform.q().y(), signalTransform.q().z(), signalTransform.q().w()) * signalDir);
...
void SignalPrototype::createGeometry(bool realScale)
{
    ...
    else
    {
        ...
        {
            std::cerr << "ERROR: no texture found named: " << fn << std::endl;
        }
    }
    if (realScale)
    {
...
void RoadSignal::setTransform(const Transform &_transform)
{
    ...
    if (roadSignalNode)
    {
        ...
        if (zOffset == 0)
        {
            ...
            roadSignalNode->setAttitude(
                toFlat * osg::Quat(signalTransform.q().x(), signalTransform.q().y(), signalTransform.q().z(), signalTransform.q().w()) * signalDir);
        }
        else
        {
...
TrafficLightSignal::TrafficLightSignal(const std::string     &setId,
                                       const std::string     &setName,
                                       const double          &setS,
                                       const double          &setT,
                                       const bool            &setDynamic,
                                       ...
                                       const int             &setType,
                                       const int             &setSubtype,
                                       const std::string     &setSubclass,
                                       const double          &setSize,
                                       const double          &setValue,
                                       const double          &setHdg,
                                       const double          &setPitch,
                                       const double          &setRoll,
